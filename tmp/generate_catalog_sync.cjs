...
const mainCategories = mainRows
  ...
  .filter((row) => row.name && row.name.toLowerCase() !== 'total');
...
for (const row of productRows) {
  ...
  const key = `${main}|||${sub}`;
  if (!subcategoryCategoryMap.has(key)) subcategoryCategoryMap.set(key, category);
}
...
const subcategories = subRows
  ...
  .filter((row) => row.mainName && row.name && row.mainName.toLowerCase() !== 'total');
